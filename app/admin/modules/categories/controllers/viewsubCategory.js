...
subcategoryListCtrl.$inject = ['CategoryServiceResolve', 'CategoryService', '$scope', '$stateParams', '$state'];

function subcategoryListCtrl(CategoryServiceResolve, CategoryService, $scope, $stateParams, $state) {
  var tlc = this;
  ...
  tlc.filterByClassification = function(classification) {
    if (!classification)
      $state.go('app.categories.subcategorylist', {}, {reload: true});
    else
      $state.go('app.categories.subcategorylist', {page: 0, items: 10, classification: classification}, {reload: true});
  };
}
