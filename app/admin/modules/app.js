...
angular.module('handyforall.admin')
            ...
            $rootScope.userPrivilages.privileges.filter(function (childs) {
              if ((childs.state === parent || childs.state === toState.name) && (childs.status[toState.action] === false || (toState.action === 'all' && childs.status.delete === false && childs.status.edit === false && childs.status.add === false && childs.status.view === false))) {
                if (!path[2]) {
                  ...
                  if (childs.childs) {
                    childs.childs.filter(function (menu) {
  ...
  .factory('myHttpInterceptor', function ($location, $rootScope, $q) {
    var timestampMarker = {
                  ...
                  user.privileges.filter(function (childs) {
                    if (childs.state === data[j].state) {
                      if (childs.status.view === false && childs.status.add === false && childs.status.edit === false && childs.status.delete === false) {
                        ...
                        data[j].childs.filter(function (menu, index) {
                          if (childs.status.view === false && menu.action === 'view') {
                            if (data[j].childs[index].state === menu.state) {
                              data[j].childs.splice(index, 1);
                            }
  ...
  .config(['$stateProvider', '$urlRouterProvider', '$httpProvider', '$urlMatcherFactoryProvider', function ($stateProvider, $urlRouterProvider, $httpProvider, $urlMatcherFactoryProvider) {
    ...
    $stateProvider
      ...
      .state('app.tasker_management.travel-arrangement', {
        url: '/travel-arrangement',
        action: 'all',
        template: '<div ui-view></div>'
      })
      .state('app.tasker_management.travel-arrangement.list', {
        url: '/list',
        action: 'all',
        controller: 'travelArrangementListCtrl',
        controllerAs: 'ctrl',
        templateUrl: 'app/admin/modules/travel-arrangement/views/travel-arrangement-list.html',
        resolve: {
          travelArrangementServiceResolve: function (TravelArrangementService) {
            return TravelArrangementService.getTravelArrangementList(10, 0);
          }
        }
      })
      .state('app.tasker_management.travel-arrangement.edit', {
        url: '/edit/:id',
        action: 'add',
        controller: 'editTravelArrangementCtrl',
        controllerAs: 'ctrl',
        ...
        resolve: {
          TravelArrangementEditResolve: function (TravelArrangementService, $stateParams) {
            if ($stateParams.id) {
              ...
              return TravelArrangementService.getTravelArrangement();
            }
          }
        }
      })
  ...
  .controller('MainCtrl', function (MainResolve, $scope, $rootScope, $cookies, MainService) {
    if (MainResolve) {
