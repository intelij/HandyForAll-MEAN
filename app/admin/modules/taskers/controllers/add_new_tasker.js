...
addNewTaskerCtrl.$inject = ['$filter', '$state', '$scope', '$modal', 'toastr', '$timeout', 'TaskersService', 'BrandService', 'taskerAddServiceResolve', 'CategoryServiceResolve', 'NgMap'];

function addNewTaskerCtrl($filter, $state, $scope, $modal, toastr, $timeout, TaskersService, BrandService, taskerAddServiceResolve, CategoryServiceResolve, NgMap) {
  ...
  TaskersService.getTravelArrangement().then(function (respo) {
    antsc.travel_arrangements = respo;
  });

  BrandService.getBrandList().then(function (response) {
    antsc.brands = response[0];
  });
    ...
    var modalInstance = $modal.open({
      ...
      resolve: {
        ...
        travel_arrangements: function () {
          return antsc.travel_arrangements;
        },
        brands: function () {
          return antsc.brands;
        },
...
angular.module('handyforall.taskers').controller('NewCategoriesModalInstanceCtrl', function (TaskersService, experiences, travel_arrangements, brands, user, categories, category, toastr, $modalInstance, $scope, defaultCurrency) {
  var nacm = this;
  nacm.user = user;
  nacm.categories = categories;
  nacm.defaultcurrency = defaultCurrency;
  nacm.experiences = experiences;
  nacm.travel_arrangements = travel_arrangements;
  nacm.brands = brands;
