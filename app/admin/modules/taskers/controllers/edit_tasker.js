...
function addTaskerCtrl(taskerAddServiceResolve, $filter, $state, TaskersService, BrandService, $scope, $modal, toastr, CategoryServiceResolve, $timeout, $http, $stateParams, $translate) {
  ...
  if (taskerAddServiceResolve) {
    ...
    atsc.mapToInput = function () {
      if ($scope.maps[0]) {
        atsc.tasker.radius = parseInt($scope.maps[0].shapes.circle.radius / atsc.radiusval, 10);
  ...
  atsc.submitTaskerEditData = function (isValid) {
  ...
  atsc.savepassword = function (valid) {
    atsc.tasker.role = "tasker";
    if (valid) {
      TaskersService.savetaskerpassword(atsc.tasker).then(function () {
  ...
  atsc.saveAvailability = function () {
    if (atsc.tasker.location === '') {
      ...
      if (atsc.tasker.location !== '') {
        TaskersService.saveAvailability(atsc.tasker).then(function () {
  ...
  atsc.saveProfile = function () {
    ...
    TaskersService.saveProfile(atsc.tasker).then(function () {
      toastr.success('Updated Successfully');
      $state.go('app.taskers.list', { page: $stateParams.page, items: $stateParams.items });
    }, function (err) {
      if (err.msg) {
  ...
  atsc.categoryModal = function (category) {
    var modalInstance = $modal.open({
      ...
      resolve: {
        ...
        user: function () {
          ...
          if (category) {
            // console.log(atsc.tasker._id, "fffffff");
    ...
    modalInstance.result.then(function (selectedCategoryData) {
      TaskersService.addCategory(selectedCategoryData).then(function () {
        toastr.success('Updated Successfully');
        $state.go('app.taskers.list', { page: $stateParams.page, items: $stateParams.items });
        atsc.addcat();
      }, function (err) {
  ...
  atsc.deletecategory = function (category) {
    ...
    modalInstance.result.then(function (deletecategorydata) {
      TaskersService.deleteCategory(deletecategorydata).then(function () {
        TaskersService.gettaskercategory(atsc.tasker._id).then(function (respo) {
          atsc.taskercategory = respo;
        });
      }, function () {});
  ...
  angular.forEach(atsc.workingDays, function (_workingDays, key) {
    angular.forEach(atsc.tasker.working_days, function (UserWorkingdays) {
      if (UserWorkingdays.day === _workingDays.day) {
  ...
  atsc.availabilityChange = function (value) {
    ...
    TaskersService.updateAvailability(atsc.data).then(function () {
  ...
  atsc.saveaccountinfo = function (isvalid, data) {
    atsc.banking.userId = $stateParams.id;

    if (isvalid) {
      TaskersService.saveaccountinfo(data).then(function () {
