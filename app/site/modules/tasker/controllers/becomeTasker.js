...
becomeTaskerCtrl.$inject = ['$scope', '$compile', 'uiCalendarConfig', '$uibModal', '$state', '$cookieStore', 'CategoryserviceResolve', '$filter', 'toastr', 'AuthenticationService', '$rootScope', '$location', 'accountService', '$translate'];

function becomeTaskerCtrl($scope, $compile, uiCalendarConfig, $uibModal, $state, $cookieStore, CategoryserviceResolve, $filter, toastr, AuthenticationService, $rootScope, $location, accountService, $translate) {
  var btc = this;
  ...
  angular.forEach(btc.categoryList, function (parentvalue) {
  ...
  btc.placeChanged = function () {
    ...
    for (var k = 0; k < locationa.address_components.length; k++) {
      var addressType = locationa.address_components[k].types[0];
      if (componentForm[addressType]) {
        var val = locationa.address_components[k][componentForm[addressType]];
  ...
  btc.filterDate = function () {
    ...
    if (calculate_age(btc.data.birthdate.month, btc.data.birthdate.date, btc.data.birthdate.year, btc.WorkingDate)) {
      ...
      $translate('YOURS AGE SHOULD BE 18').then(function (headline) { toastr.error(headline); }, function (error) { console.error(error); });
    }
  };
  ...
  btc.emailchange = function (email) {
    if (email === undefined) {
      $translate('INVALID EMAIL ID').then(function (headline) { toastr.error(headline); }, function (error) { console.error(error); });
    } else {
      AuthenticationService.checktaskeremail(email).then(function (err) {
        if (err.message === 'Email Exist') {
          btc.booleanValue = true;
          $translate('SORRY EMAIL ID ALREADY EXIST').then(function (headline) { toastr.error(headline); }, function (error) { console.error(error); });
        } else if (err.message === 'Email not exist') {
          btc.booleanValue = false;
          $translate('VALID EMAIL ID').then(function (headline) { toastr.error(headline); }, function (error) { console.error(error); });
        }
      });
    }
  };
  ...
  btc.taskernamechange = function (tasker) {
    if (tasker === undefined) {
      $translate('INVALID TASKER').then(function (headline) { toastr.error(headline); }, function (error) { console.error(error); });
    } else {
      AuthenticationService.taskername(tasker).then(function (err) {
        if (err.message === 'Tasker Exist') {
          btc.booleanValue = true;
          btc.data.username = '';
          $translate('SORRY USERNAME ALREADY EXIST').then(function (headline) { toastr.error(headline); }, function (error) { console.error(error); });
        } else if (err.message === 'Tasker not exist') {
          btc.booleanValue = false;
          $translate('VALID USERNAME').then(function (headline) { toastr.error(headline); }, function (error) { console.error(error); });
        }
      });
    }
  };
  ...
  btc.phonechange = function (phone) {
    if (phone === undefined) {
      $translate('INVALID PHONE NUMBER').then(function (headline) { toastr.error(headline); }, function (error) { console.error(error); });
    } else {
      AuthenticationService.taskerphone(phone).then(function (err) {
        if (err.message === 'Phone Exist') {
          btc.data.phone = { "code": "", "number": "" };
          btc.booleanValue = true;
          $translate('SORRY PHONE NUMBER ALREADY EXIST').then(function (headline) { toastr.error(headline); }, function (error) { console.error(error); });
        } else if (err.message === 'Phone not exist') {
          btc.booleanValue = false;
          $translate('VALID PHONE NUMBER').then(function (headline) { toastr.error(headline); }, function (error) { console.error(error); });
        }
      });
    }
  };
          ...
          if (typeof btc.data.phone !== "undefined" && typeof btc.data.phone.code !== "undefined" && typeof btc.data.phone.number !== "undefined") {
            accountService.checkphoneno(btc.data.phone).then(function (response) {
              if (response.length > 0) {
                $translate('Phone Number Already Existed!').then(function (headline) { toastr.error(headline); }, function (error) { console.error(error); });
              } else {
                $cookieStore.put('TaskerData', btc.data);
            ...
            $translate('Phone Number is required!').then(function (headline) { toastr.error(headline); }, function (error) { console.error(error); });
          }
        } else {
          $translate('Invalid City').then(function (headline) { toastr.error(headline); }, function (error) { console.error(error); });
        }
      }
    } else {
      $translate('PLEASE FILL ALL MANDATORY FIELDS').then(function (headline) { toastr.error(headline); }, function (error) { console.error(error); });
    }
  };
  ...
  btc.mapToInput = function () {
    if ($scope.maps[0]) {
      btc.data.radius = parseInt($scope.maps[0].shapes.circle.radius / btc.radiusval, 10);
  ...
  */
  ...
  btc.TaskerRegister = function (valid) {
    ...
    if (valid > 0) {
      ...
      $translate('SELECT ATLEAST ONE OF THE SKILLS').then(function (headline) { toastr.error(headline); }, function (error) { console.error(error); });
    }
  };
  ...
  btc.addskills = function (valid) {
    if (valid) {
      ...
      var DefaultCurrency = $scope.DefaultCurrency;
    ...
    if ((btc.data.working_days[6].not_working === false || btc.data.working_days[5].not_working === false ||
      ...
      if (calculate_age(btc.data.birthdate.month, btc.data.birthdate.date, btc.data.birthdate.year, btc.WorkingDate)) {
        if (btc.data.location === '') {
          $translate('INVALID WORK LOCATION').then(function (headline) { toastr.error(headline); }, function (error) { console.error(error); });
        } else {
          $cookieStore.put('TaskerData', btc.data);
          $state.go('becometasker.' + btc.data.next, {}, { reload: false });
        }
      ...
      $translate('PLEASE SELECT YOUR WORKING DAYS').then(function (headline) { toastr.error(headline); }, function (error) { console.error(error); });
    }
  };
...
angular.module('handyforall.becometasker').controller('ModalInstanceWorkingDayCtrl', function ($scope, $uibModalInstance, DaysData, WorkingDays, workingTimes, selectedIndex, accountService) {
...
angular.module('handyforall.becometasker').controller('ModalInstancecategory', function (accountService, $uibModalInstance, experiences, user, toastr, categories, category, selectedCat, defaultcurrency) {
  var acm = this;
  ...
  acm.onChangeCategoryChild = function (_category) {
    accountService.getChild(_category).then(function (response) {
