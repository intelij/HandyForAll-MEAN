module.exports = function (io) {
  ...
  chat.on('connection', function (socket) {
    ...
    /* socket.on('liveTracking', function (data) {
      ...
      db.GetOneDocument('task', { '_id': data.task }, {}, extension, function (err, response) {
        if (response) {
          socket.emit('liveTracking', response);
        }
      });
    }) */
  ...
  notify.on('connection', function (socket) {
    ...
    socket.on('tasker tracking', function (data) {
      ...
      if (data.bearing) {
        data = { user: data.tasker, latLng: { lat: data.lat, lng: data.lng } }
      }
      notify.emit('webtaskertracking', data);
    });
