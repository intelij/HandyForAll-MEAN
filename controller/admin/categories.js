...
module.exports = function () {
  ...
  controller.getTree = function (req, res) {
    const makeTree = (catList) => {
      const tree = [];
      const firstLevel = [];
      for (const value of catList) {
        if (!value.parent) {
          firstLevel.push(value);
        }
      }
      const getChild = (item) => {
        const childs = [];
      ...
      for (const firstValue of firstLevel) {
        tree.push(firstValue);
        // calc second level
        const secondLevel = getChild(firstValue);
        for (const secValue of secondLevel) {
          tree.push(secValue);
          // calc third level
          const thirdLevel = getChild(secValue);
          for (const thirdValue of thirdLevel) {
            tree.push(thirdValue);
            // calc forth level
            const forthLevel = getChild(thirdValue);
            for (const forthValue of forthLevel) {
              tree.push(forthValue);
              // calc fifth level
              // const fifthLevel = getChild(forthValue);
              // for (const fifthValue of fifthLevel) {
              //   tree.push(fifthValue);
              // }
            }
          }
        }
