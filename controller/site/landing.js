...
module.exports = function () {
  ...
  router.getlandingdata = function (req, res) {
    async.parallel({
      filtered_service_categories: function (callback) {
        db.GetDocument('category', { 'status': 1, 'classification': 'service', 'parent': { $exists: false } }, {}, {sort: { name : 1 }}, function (err, categories) {
          if (err) {
            ...
            var arr_categories = [];
            for (var i = 0, len = Math.min(6, categories.length); i < len; i++) {
              arr_categories.push(categories[i])
            }

            callback(err, arr_categories);
          }
        });
      },
      ...
      product_categories: function (callback) {
        db.GetDocument('category', { 'status': 1, 'classification': 'product', 'parent': { $exists: false } }, {}, {sort: { name : 1 }}, function (err, categories) {
          if (err) {
            callback(err, categories);
          } else {
      ...
      event_categories: function (callback) {
        db.GetDocument('category', { 'status': 1, 'classification': 'event', 'parent': { $exists: false } }, {}, {sort: { name : 1 }}, function (err, categories) {
          if (err) {
            callback(err, categories);
          } else {
      ...
      career_categories: function (callback) {
        db.GetDocument('category', { 'status': 1, 'classification': 'career', 'parent': { $exists: false } }, {}, {sort: { name : 1 }}, function (err, categories) {
          if (err) {
            callback(err, categories);
          } else {
      ...
      if (err || !result) {
        ...
        res.send({
          'filtered_service_categories': result.filtered_service_categories,
          'PostHeader': result.Postheader,
          'Settings': result.Settings,
          'slider': result.slider,
          'Paymentprice': result.Paymentprice,
          'Peoplecomment': result.Peoplecomment,
          ...
          'career_categories': result.career_categories
        });
      }
    });
  }
